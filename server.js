...
const CFG = {
  ...
  SUPER_ADMIN_PWD   : process.env.SUPER_ADMIN_PWD   || 'ChangeMe2025!',
...
// ─────────────────────────────────────────────
function cloudinaryUpload(base64Data, folder = 'covenantcrest', publicId = null) {
  ...
  addField('file',      `data:image/jpeg;base64,${base64Data}`);
  ...
  return new Promise((resolve, reject) => {
    const req = https.request({
      ...
      path    : `/v1_1/${CFG.CLOUDINARY_CLOUD}/image/upload`,
      method  : 'POST',
      headers : {
...
app.post('/api/jobs', requireAuth, async (req, res) => {
  ...
  const job = {
    ...
    desc     : sanitise(desc     || '', 2000),
    req      : sanitise(requirements || '', 1000),
